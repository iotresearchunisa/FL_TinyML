...
const int numberInputLayer = 15;
const int numberOutputLayer = 1;
const int numberHiddenLayer = 2;
const int numberHiddenLayerNeurons = 12;
const int numberEpochs = 70;

String pathTrain = "";
String pathResult = "";
String pathSaveWeights = "";

const String pathTest = "/dataset/car/test.csv";
...
bool initFiles(){
    pathTrain = "/dataset/car/train" + String(nodeNumber) + ".csv";
    pathResult = "/resultcar" + String(nodeNumber) + ".csv";
    pathSaveWeights = "/carWeights" + String(nodeNumber) + ".txt";
...
void execute(int epoch) {
    ...
    if (!trainingDisabled ) {
        ...
        Serial.print("\tTR: ");Serial.print(getAccuracy());Serial.print("\t");
    }
    ...
    if(!testingDisabled){
      ...
      printTrainingTimer(resultFile);printResult(resultFile);
      Serial.print("\tTE: ");Serial.print(getAccuracy());Serial.print("\n");
    }

    if (!trainingDisabled) {
        saveWeightsJson(ann, pathSaveWeights, getNumberDataset(), getAccuracy());
    }
}
...
void trainingMode() {
    ...
    if (useTransferLearning){
        ...
        loadWeightsJson(ann, "/carWeights_start.txt");
